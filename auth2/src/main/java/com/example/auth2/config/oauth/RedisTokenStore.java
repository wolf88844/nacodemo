...
public class RedisTokenStore implements TokenStore {
    ...
    public OAuth2Authentication readAuthentication(String token) {
        ...
        try{
            bytes = connection.get(this.serializeKey(AUTH+token));
        }finally {
            connection.close();
        }
        OAuth2Authentication oAuth2Authentication = this.deserializeAuthentication(bytes);
        return oAuth2Authentication;
    }

    @Override
    public void storeAccessToken(OAuth2AccessToken token, OAuth2Authentication authentication) {
        byte[] serializeAccessToken = serialize(token);
        byte[] serializeAuth = serialize(authentication);
        ...
        try{
            ...
            connection.stringCommands().set(accessKey,serializeAccessToken);
            connection.stringCommands().set(authKey,serializeAuth);
            connection.stringCommands().set(authToAccessKey,serializeAccessToken);
            if(!authentication.isClientOnly()){
                connection.rPush(approvalKey,serializeAccessToken);
            }
            connection.rPush(clientId,serializeAccessToken);
            ...
            if(refreshToken != null && refreshToken.getValue()!= null){
                ...
                connection.stringCommands().set(refreshToAccessKey,auth);

                byte[] accessToRefreshKey = serializeKey(ACCESS_TO_REFRESH + token.getValue());
                connection.stringCommands().set(accessToRefreshKey, refresh);
    ...
    public OAuth2Authentication readAuthenticationForRefreshToken(String token){
        RedisConnection con = this.getConnection();
        try{
            byte[] bytes = con.get(serializeKey(REFRESH_AUTH + token));
    ...
    public OAuth2AccessToken getAccessToken(OAuth2Authentication authentication) {
        ...
        byte[] serializeKey = this.serializeKey(AUTH_TO_ACCESS + key);
        byte[] bytes = null;
